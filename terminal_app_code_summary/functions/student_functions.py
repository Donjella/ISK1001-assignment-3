...
def add_student(students, classrooms):
    """
    The add_student functions adds a new student to the system.
    ...
    Example of how it works/is used:
        ...
        2. Validates user input to ensure correctness and consistency together with exception handling.
           Exception handling captures and handles invalid inputs such as valid names using methods such as isalpha() 
           and safely exit function if input in interrupted.
    ...
    while True:
        try:
            fname = input("Enter student's first name: ").strip().capitalize() # prompts for first name input
